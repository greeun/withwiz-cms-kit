import { buildPaginatedResult } from '@withwiz/cms-kit/types/common';

describe('buildPaginatedResult', () => {
  it('CMS-P-01: 정상 계산 - totalPages, hasMore', () => {
    const result = buildPaginatedResult(['a', 'b'], 10, 1, 5);
    expect(result.pagination.totalPages).toBe(2);
    expect(result.pagination.hasMore).toBe(true);
  });

  it('CMS-P-02: 마지막 페이지 → hasMore=false', () => {
  ...
  it('CMS-P-06: items 배열 참조 보존', () => {
    const items = [{ id: 1 }, { id: 2 }];
    const result = buildPaginatedResult(items, 2, 1, 10);
    expect(result.items).toBe(items);
  });

  it('CMS-P-07: page/pageSize 정확한 반환', () => {
