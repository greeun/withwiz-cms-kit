...
/**
 * CMS-RKS — storage-key sanitization (spec.md §4.6 / AC-4.6.5 / Sprint 1 S5).
 *
...
describe('r2 storage-key sanitization (CMS-RKS)', () => {
  beforeEach(() => {
    vi.resetModules();
    mockSend.mockClear();
    putArgs.length = 0;
  ...
  it('CMS-RKS-01: uploadToR2 rejects all five malicious key classes (S3 cmd NOT constructed)', async () => {
  ...
  it('CMS-RKS-02: deleteFromR2 rejects all five malicious key classes (S3 cmd NOT constructed)', async () => {
    const { deleteFromR2 } = await import('@withwiz/cms-kit/utils/r2-storage');
    for (const bad of MALICIOUS) {
      await expect(deleteFromR2(bad)).rejects.toThrowError(/@withwiz\/cms-kit/);
    }
    expect(delArgs).toHaveLength(0);
    expect(mockSend).not.toHaveBeenCalled();
  });

  it('CMS-RKS-03: uploadToR2 benign keys reach S3 with Key byte-identical (negative control)', async () => {
    const { uploadToR2 } = await import('@withwiz/cms-kit/utils/r2-storage');
  ...
  it('CMS-RKS-04: deleteFromR2 benign keys reach S3 with Key byte-identical (negative control)', async () => {
    const { deleteFromR2 } = await import('@withwiz/cms-kit/utils/r2-storage');
  ...
  it('CMS-RKS-06: each malicious class individually (delete) — discrete coverage', async () => {
    const { deleteFromR2 } = await import('@withwiz/cms-kit/utils/r2-storage');
    for (const bad of MALICIOUS) {
      delArgs.length = 0;
      mockSend.mockClear();
